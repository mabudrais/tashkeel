...
var
  FileName, cFile: string;
  FileNo: integer;
  fillst: TStringList;
begin
  FApplicationName := ExtractFileName(ParamStr(0));
  FApplicationPath := ExtractFilePath(ParamStr(0));
  FileName := FApplicationPath + 'log' + PathDelim + ChangeFileExt(
    FApplicationName, '.log');
  FileNo := 0;
  while FileExists(FileName) do
  begin
    Inc(FileNo);
    FileName := FApplicationPath + 'log' + PathDelim + IntToStr(FileNo) +
      ChangeFileExt(FApplicationName, '.log');
  end;
  if FileNo > 100 then
  begin
    fillst := FindAllFiles(FApplicationPath + 'log');
    for cFile in fillst do
      DeleteFile(cFile);
  end;
...
var
  S: string;
begin
  S := '*** ERROR *** : @ ' + TimeToStr(Time) + ' MSG : ' + ErrorMessage +
    ' IN : ' + Location + #13#10;
...
var
  S: string;
begin
  S := 'STATUS INFO : @ ' + TimeToStr(Time) + ' MSG : ' + StatusMessage +
    ' IN : ' + Location + #13#10;
  WriteLn(FFileHandle, S);
  Flush(FFileHandle);
end;
...
var
  S: string;
begin
  S := '=== WARNING === : @ ' + TimeToStr(Time) + ' MSG : ' +
    WarningMessage + ' IN : ' + Location + #13#10;
  WriteLn(FFileHandle, S);
  Flush(FFileHandle);
end;
